namespace RemoveDuplicatesArraySorted
{
  public class DuplicateRemover
  {
    ...
    public int DuplicateRemoverSolution1(int[] nums)
    {
      ...
      */      
      ...
      while (i < lastElementPosition)
      {
        ...
        {
          // all elements after position i, were the same, no swapping necessary
          break;
        }
        lastSwappedValue = SwapElementsInArray(nums, j, i + 1);        
        if (j == lastElementPosition)
        {
          // you've swapped i+1 with j. so total number of elements will be i+2.
          return i + 2;
        }
        i++;
      }
    ...
    {
      int j = startIndex + 1;
      while (j < nums.Length && (nums[j] <= nums[startIndex] || nums[j] <= lastSwapped))
      {
        j++;
      }
      return j;
    ...
    public int DuplicateRemoverSolution2(int[] nums)
    {
      if (nums.Length == 0)
        return 0;
